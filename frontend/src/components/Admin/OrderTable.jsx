...
const OrderTable = () => {
    ...
    const columns = [
                    ...
                    <>
                        {
                            params.row.status === "Delivered" ? (
                                ...
                                <span className="text-sm bg-purple-100 p-1 px-2 font-medium rounded-full text-purple-800">{params.row.status || "Processing"}</span>
                            )
                        }
                    </>
                )
        ...
        },
        {
            ...
            renderCell: (params) => {
                return (
                    <span>{params.row.amount ? params.row.amount.toLocaleString() + "đ" : "N/A"}</span>
                );
            },
        },
        {
    ...
    orders && orders.forEach((order) => {
        console.log(order); // Log the order to inspect its structure

        // Check for orderItems and totalPrice
        const itemsQty = order.orderItems ? order.orderItems.length : 0;
        const amount = order.totalPrice ? order.totalPrice : 0;

        console.log("Items Qty:", itemsQty); // Log Items Qty
